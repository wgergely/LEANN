...
class PythonTachProvider(BaseAnalysisProvider):
    ...
    def _run_map(self, direction: str = "dependencies") -> dict[str, list[str]]:
        ...
        try:
            ...
            return {
                k.replace("\\", "/"): [p.replace("\\", "/") for p in v] for k, v in data.items()
            }
        except Exception:
            return {}
    ...
    def _get_report(self, rel_path: str, mode: str) -> list[str]:
        ...
        try:
            result = subprocess.run(args, cwd=self.project_root, capture_output=True, text=True)
            lines = []
            capture = False
            for line in result.stdout.splitlines():
                ...
                if not line:
                    continue
                if any(
                    h in line for h in ["Dependencies of", "Usages of", "External Dependencies"]
                ):
                    capture = True
                    continue
