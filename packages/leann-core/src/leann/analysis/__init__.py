...
class CodeAnalyzer:
    ...
    def analyze(self, code: str, file_path: str = "") -> dict[str, Any]:
        ...
        try:
                    ...
                    for imp in imports:
                        ...
                        for cand in candidates:
                            if cand.exists() and cand.is_file():
                                try:
                                    resolved_imports[imp] = str(
                                        cand.relative_to(search_root)
                                    ).replace("\\", "/")
                                    break
                                except ValueError:
            ...
            # 6. Provider-based analysis (Enrichment)
            provider_data = {}
            if file_path:
                try:
                    path_obj = Path(file_path).resolve()
                    # Try to find project root (look for .leann or .git)
                    search_root = path_obj.parent
                    found_root = None
                    for _ in range(7):
                    ...
                    if found_root:
                        ...
                        if provider:
                            provider_data = provider.get_file_context(path_obj)
                            result["provider_data"] = provider_data
                except Exception as e:
                    logger.debug(f"Provider analysis skipped for {file_path}: {e}")

            # 7. Context Block Generation
            context_parts = []
            ...
            # Inject Provider Data (TACH etc.)
            if provider_data:
                if provider_data.get("detailed_dependencies"):
                    deps = provider_data["detailed_dependencies"][:5]
                    context_parts.append("Detailed Dependencies: " + "; ".join(deps))
                if provider_data.get("external"):
                    exts = provider_data["external"][:3]
                    context_parts.append("External: " + ", ".join(exts))
    ...
    def get_semantic_chunks(
        ...
        try:
            ...
            if truncated_count > 0:
                logger.info(
                    f"Refined {truncated_count} chunks to stay within 2048 token limit for {file_path}"
                )
