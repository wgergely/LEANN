...
def create_text_chunks(
    ...
    def process_docs_parallel(docs, chunk_func, **kwargs):
        ...
        logger.info(
            f"Parallelizing {len(docs)} docs across {num_workers} workers (batch_size={batch_size})"
        )
        ...
        try:
            ...
            pbar = tqdm(
                total=len(batches),
                desc="Processing AST chunks (parallel)",
                unit="batch",
                leave=False,
            )
        except ImportError:
        ...
        with concurrent.futures.ProcessPoolExecutor(
            ...
            for future in concurrent.futures.as_completed(future_to_batch):
                ...
                try:
                    ...
                    logger.error(
                        f"Parallel worker failed on batch starting with {batch_sample}: {e}"
                    )
