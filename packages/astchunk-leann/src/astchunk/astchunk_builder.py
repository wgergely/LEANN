...
# check availability of java/csharp bindings
try:
...
class ASTChunkBuilder:
    ...
    def __init__(self, **configs):
        ...
        elif self.language == "java" and tsjava:
            lang = ts.Language(tsjava.language())
            self.parser = ts.Parser(lang)
        elif self.language == "csharp" and tscsharp:
            lang = ts.Language(tscsharp.language())
            self.parser = ts.Parser(lang)
        elif self.language == "typescript":
            lang = ts.Language(tstypescript.language_tsx())
            self.parser = ts.Parser(lang)
        elif self.language == "javascript":
            # Explicit javascript support using typescript/tsx parser or js parser if preferred
            lang = ts.Language(tsjavascript.language())
            self.parser = ts.Parser(lang)
        else:
            # Fallback or error
            if self.language in ["java", "csharp"]:
                raise ValueError(f"Language binding for {self.language} not installed.")
    ...
    def assign_nodes_to_windows(
        self, nodes: list[ts.Node], nws_cumsum: np.ndarray, ancestors: pyrsistent.pvector
    ) -> Generator[list[ASTNode], None, None]:
        """
        ...
        for node in nodes:
            ...
            if (len(current_window) == 0 and node_exceeds_limit) or (
                ...
                if node_exceeds_limit:
                    ...
                    child_windows = list(
                        self.assign_nodes_to_windows(node.children, nws_cumsum, childs_ancestors)
                    )
                    if child_windows:
    ...
    def convert_windows_to_chunks(
        self, ast_windows: list[list[ASTNode]], repo_level_metadata: dict, chunk_expansion: bool
    ) -> list[ASTChunk]:
        """
    ...
    def chunkify(self, code: str, **configs) -> list[dict]:
        ...
        ast_windows = list(self.assign_tree_to_windows(code=code, root_node=ast.root_node))
